...
class OutputCSV:
    def __init__(self, old_csv: Path):
        ...
        self.names_dict = {consts.str_cleanup(row[self.column_name]): row for row in self.data[1:]}

    def save(self, new_csv: Path):
        with new_csv.open('w', encoding='utf-8') as new_csv_file:
            newcsv = csv.writer(new_csv_file, delimiter=',', quotechar='"')
    ...
    def save_student(self, student: consts.StudentGrade) -> bool:
        row = self.names_dict.get(consts.str_cleanup(student.name), None)
        if row is None:
            return False
